...
@Component({
  selector: 'app-camp-surgery-patients-reports',
  templateUrl: './camp-surgery-patients-reports.component.html',
  styleUrls: ['./camp-surgery-patients-reports.component.less'],
  providers: [
...
export class CampSurgeryPatientsReportsComponent implements OnInit {

  @ViewChild('CampSurgeryPatientsReports') Form: NgForm
  ...
  ngOnInit()
  {
    ...
    var Pathname = "Camp/CampSurgeryPatientsReports";
    var n = Pathname;
    ...
    if (sstring == false) {
      if (Objdata.find(el => el.Parentmoduledescription === Pathname)) {
        ...
        this.commonService.getListOfData('Common/Getlogdetails/' + localStorage.getItem("CompanyID") + '/' + localStorage.getItem("Doctorname") + '/' + "CampSurgeryPatientsReports").subscribe(data => {
  ...
  onSubmit(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.commonService.getListOfData('Help/getCampSurgerypatients/' + this.M_CampName.Value + '/' + Fromdate + '/' + Todate + '/' + parseInt(localStorage.getItem("CompanyID"))).subscribe(data => {
        if (data.CampSurgerypatients.length > 0) {
          debugger;
       
          this.CampSurgerypatients = data.CampSurgerypatients;
         this.CampwiseSurgeryCount = data.CampwiseSurgeryCount
  ...
  ConvertPDF() {
    ...
    var Stringfydfata = JSON.stringify(this.CampSurgerypatients);
    ...
    var documentDefinition = {
      info: {
        title: 'List of camp Surgery patients Details',
      },
      pageSize: {
        width: 1350,
        height: 1300
    ...
    pdfMake.createPdf(documentDefinition).download('List of camp Surgery patients Details.pdf');
  }
  ...
  ConvertPDF1() {
    debugger;
    var companyname = localStorage.getItem("Companyname");
    var Stringfydfata = JSON.stringify(this.CampwiseSurgeryCount);
    var objdata = JSON.parse(Stringfydfata);
    ...
    var documentDefinition = {
      info: {
        title: 'List of Campwise Surgery count patients Details',
      },
      pageSize: {
        width: 1350,
        height: 1300
    ...
    pdfMake.createPdf(documentDefinition).download('Campwise Surgery count Details.pdf');
  }
  ...
  ConvertEXCEL() {
    ...
    XLSX.writeFile(wb, "List of camp Surgery patients.xlsx");
  }
  ...
  ConvertEXCEL1() {
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Patient Details');
    XLSX.writeFile(wb, "List of Campwise Surgery count patients.xlsx");
  }
  Getformaccess() {
    debugger;
    var Pathname = "Camp/CampSurgeryPatientsReports";
