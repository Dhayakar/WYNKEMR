...
@Component({
  ...
  styleUrls: ['./bookedappointments.component.less']
})
export class BookedappointmentsComponent implements OnInit {
  ...
  APoointmentdisplayedColumns: string[] = ['AppointNAme', 'AppointAGe', 'AppointGender', 'AppointDate', 'AppointcreatedDate', 'Appoinytstatusss', 'Appoinytbookedby', 'Appoinytpreferreddoctor', 'AppointmentCancelMY'];
  EyeDoctors;
  hideviewdrop = false;
  ...
  ngOnInit() {
    const Pathname = 'AppointmentLazy/BookedAppts';
  ...
  Getformaccess() {
    debugger;
    this.commonService.getListOfData('Common/GetAccessdetailsstring/' + localStorage.getItem('CompanyID') + '/' + localStorage.getItem('userroleID') + '/' + 'AppointmentLazy/BookedAppts').subscribe(data => {
      debugger;
      this.accessdata = data.GetAvccessDetails;
      this.backdrop = 'block';
        ...
        .subscribe(data => {
          if (data.Success == true) {
            ...
            this.router.navigateByUrl('/dash', { skipLocationChange: true }).then(() => {
              ...
              this.router.navigate(['AppointmentLazy/BookedAppts']);
            });
